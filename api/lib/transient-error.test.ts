...
describe("isAutoMergeNotAllowedError", () => {
  ...
  it("returns false for a plain Error with the old string sentinel", () => {
    // The pre-fix code string-matched .message for "PullRequestAutoMergeNotAllowed",
    // which always returned false against real GitHub GraphqlResponseErrors.
    // This test confirms the old check is gone and only real errors match.
