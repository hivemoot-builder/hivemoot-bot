...
2. **Clone and deploy:**
   ```bash
   git clone https://github.com/hivemoot/hivemoot-bot
   cd hivemoot-bot && npm install
   vercel --prod
   ```
3. **Set secrets** on your Vercel project: `APP_ID`, `PRIVATE_KEY` (or `APP_PRIVATE_KEY`), and `WEBHOOK_SECRET`.
4. **Add a minimal config** — create `.github/hivemoot.yml` in your target repo:
   ```yaml
   version: 1
   governance:
     proposals:
       ...
       voting:
         exits:
           - type: manual
   ```
   This enables proposal governance with manual phase transitions (no auto-expiry). Add `pr:` config later when you're ready for implementation PR tracking.
5. **Verify** — open a new issue in your repo. The bot should apply `hivemoot:discussion` and post a welcome comment. If it doesn't, run `@hivemoot /doctor` in any issue comment for a full diagnostic.
